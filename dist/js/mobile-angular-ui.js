...
`.active` classes are added/removed each time one of `$stateChangeSuccess` or `$includeContentLoaded` is fired.
...
(function () {
  ...
  .run([
      ...
      function($rootScope, $window, $document, $location){
        ...
        $rootScope.$on('$stateChangeSuccess', setupActiveLinks);
...
(function() {
   ...
   *
   * @param {string} muiState The shared state id
   * @param {expression} [uiDefault] the default value
   *
   * @directive muiState
   */
  module.directive('muiState', [
    'SharedState',
    function(SharedState){
      return {
        ...
        link: function(scope, elem, attrs){
          var id               = attrs.muiState || attrs.id,
...
(function () {
   ...
   *
   * Modals are basically the same of Bootstrap 3 but you have to use muiState
